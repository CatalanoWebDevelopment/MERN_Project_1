...
router.delete('/:id', auth, async (req, res) => {
    try {
        ...
        // Check to see if the User who deletes the post is the user who created the post
...
router.put('/like/:id', auth, async (req, res) => {
    try {
        ...
        if (post.likes.filter(like => like.user.toString() === req.user.id).length > 0) {
            return res.status(400).json({ msg: 'Post has already been liked by this user.' });
        }
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) => {
    try {
        ...
        // Make sure user deleting comment is user who created comment
